...
def add_tag_spend(df):
    ...
    'banking charges', and multiple similar categories for clothes.
    """
    df['tag_spend'] = df.tag_auto.where(df.tag_group.eq('spend'), np.nan)

    d = {
        'bank charges': ['banking charges'],
        'beauty product and services': [
            'beauty products',
            'beauty treatments'
        ],
        'child expenses': [
